...
(*IMPORTANT: FindRoot errors for precision larger >= 300 not really understood (sometimes it just works with higher precision like 301 but 300 is a problem*)


IntScal[R_,\[ScriptL]_,mu_,a_]:=Block[{l=\[ScriptL],dLd\[Sigma]h,dLd\[Sigma]\[Infinity],dEd\[Sigma]\[Infinity],dEd\[Sigma]h,m=mu,orbit,mode,s,n,k,\[Omega]},
...
x = Partition[Flatten[ParallelTable[{arrayp[a][[ics]],Sum[2*IntScal[arrayp[a][[ics]],multipoles[[l,1]],multipoles[[l,2]],a],{l,1,Length[multipoles]}]},{ics,1,Length[arrayp[a]]}]],5];
Return[x];
];
