...
BeginPackage["EmriInspiralMassiveFisher`"];
...
dEGRdr = Import["/THEORY/USERS/susanna.barsanti/DATA/FISHERMASSIVE/dEdrGR09HPI.mx"];
(*dEddr = Import["/THEORY/USERS/susanna.barsanti/DATA/FISHERMASSIVE/dEdrdmu09HPI.mx"];*)


datisusi = Import["/THEORY/USERS/susanna.barsanti/DATA/FISHERMASSIVE/data_mu0018_tot3.wdx"];
gridsusi = 
  Table[{{datisusi[[i, 1]], datisusi[[i, 2]], datisusi[[i, 3]]}, 
    datisusi[[i, 4]] + datisusi[[i, 5]]}, {i, 1, Length[datisusi]}];



dEddr = Interpolation[gridsusi];

Print["scal=",Precision[dEddr[3.4`300, 0.9`300, 0.018`300]]];
Print["mas=",Precision[dEddr[2.42`300, 0.9`300, 0.018`300]]];
Print["scal=",dEddr[2.42`300, 0.9`300, 0.018`300]];


Print["grav=",Precision[dEGRdr[3.4`300, 0.9`300]]];
Print["grav=",Precision[dEGRdr[2.42`300, 0.9`300]]];
Print["grav=",dEGRdr[2.42`300, 0.9`300]];
...
findr0[m1_,m2_,spin_,\[Alpha]_,massphi_,rend_,T_]:=Block[{q=m2/m1,factsec,factday,fact,tend,rini,r,\[ScriptCapitalE],eqns,t},

\[ScriptCapitalE][r_] = dEGRdr[r,spin]+\[Alpha]^2 dEddr[r,spin,massphi];
Print["callingr0"]; 		
factsec = cc/(m1 msun);
...
OrbitalEvoT[m1_,m2_,spin_,\[Alpha]_,massphi_,r0_,\[Phi]0_,T_]:=Block[{solr,t,sol,r,\[Psi],factday,factsec,tend,q=m2/m1,fact,eqns,IC,\[ScriptCapitalE]},
...
\[Phi] = \[Phi]T + ArcTan[2Sin[\[Theta]s]Sin[\[Phi]T-\[Phi]s],Sqrt[3]Cos[\[Theta]s]+Sin[\[Theta]s]Cos[\[Phi]T-\[Phi]s]]
+Which[pol==1,0,pol==2,-\[Pi]/4,pol!=1\[Or]pol!=2,Print["BadPatternFunctionOption"];Exit[]];
...
dopplerf[m1_,m2_,spin_,\[Alpha]_,massphi_,r0_,\[Phi]0_,\[Theta]s_,\[Phi]s_,T_,t_]:=Block[{M,\[Mu],fdopp,\[Omega],x,evolution,\[Omega]D,fD},

M = m1 msun; (* total mass *)
\[Mu] = m2 msun; (* reduced mass *)

evolution = OrbitalEvoT[m1,m2,spin,\[Alpha],massphi,r0,\[Phi]0,T];
...
emritemplate[m1_,m2_,spin_,\[Alpha]_,massphi_,r0_,\[Phi]0_,\[Theta]s_,\[Phi]s_,\[Theta]k_,\[Phi]k_,T_,t_]:=Block[{evolution,\[Phi],\[Mu],M,\[Phi]D,r,\[Omega],LN},
...
htime[m1_,m2_,spin_,\[Alpha]_,massphi_,r0_,\[Theta]s_,\[Phi]s_,\[Phi]0_,\[Theta]k_,\[Phi]k_,observingtime_]:=Block[{win,nn,timevec,T,\[Delta]t,t,
...
rmax = OrbitalEvoT[m1,m2,spin,\[Alpha],massphi,r0,\[Phi]0,observingtime][[2]][T]; (* final radius after T *)	
risco = KerrIsco[spin]; (* isco radius/M *)
fdopp = dopplerf[m1,m2,spin,\[Alpha],massphi,r0,\[Phi]0,\[Theta]s,\[Phi]s,observingtime,T]; (* max fdop @ rmax *)
...
If[fsampling < fmax, Print["Sampling freq smaller than fmax"];Exit[]];
If[1/2/\[Delta]t < fsampling, Print["Nymquist criterion failed"];Exit[]]; (*fNy = fs/2 > fmax *)
(* sampling is alwasy up to the folding freq fs/2 = N/T/2 = 1/2/\[Delta]t= fNy*)

timevec =  N[Table[i \[Delta]t,{i,0,nn-1}],precision];

If[timevec[[-1]]/day != observingtime, Print["end time \[NotEqual] T"];Exit[]];

{hplus[t_],hcross[t_]} = emritemplate[m1,m2,spin,\[Alpha],massphi,r0,\[Phi]0,\[Theta]s,\[Phi]s,\[Theta]k,\[Phi]k,observingtime,t]; (* analytical waveform *)
...
If[Length[hIf]!=Length[hIIf],Print["DIFFERENT # OF SAMPLES"];Exit[]];
...
If[1/2/\[Delta]t != freqVec[[-1]], Print["fNy \[NotEqual] fmax"];Exit[]];
...
snrrwave[m1_,m2_,spin_,\[Alpha]_,massphi_,r0_,\[Phi]0_,\[Theta]s_,\[Phi]s_,\[Theta]k_,\[Phi]k_,dL_,T_,WDT_]:=Block[{fmin=10^-4,rmax,fdopp,fmax,d,\[Rho],hI,hII,timevec,fact,hp,hx,FpI,FxI,FpII,FxII,hIt,hIIt},

d=10^6 dL pc; (* Luminosity distance *)

{timevec,hIt,hIIt}=htime[m1,m2,spin,\[Alpha],massphi,r0,\[Theta]s,\[Phi]s,\[Phi]0,\[Theta]k,\[Phi]k,T];

{hI,hII} = fftwave[hIt,hIIt,timevec];

rmax = OrbitalEvoT[m1,m2,spin,\[Alpha],massphi,r0,\[Phi]0,T][[2]][T day]; (* final radius after T *)	
fdopp = dopplerf[m1,m2,spin,\[Alpha],massphi,r0,\[Phi]0,\[Theta]s,\[Phi]s,T,T day]; (* max fdop @ rmax *)
fmax = cc KerrOrbit[spin,rmax][[1]]/(m1 msun \[Pi])+fdopp; (* f @ rmax + fdop @ rmax *)
...
derwave[\[Epsilon]_,m1_,m2_,spin_,\[Alpha]_,massphi_,r0_,\[Phi]0_,\[Theta]s_,\[Phi]s_,\[Theta]k_,\[Phi]k_,dL_,observingtime_]:=Block[{timevec,vecder,
...
npar = Length[{m1,m2,spin,\[Alpha],massphi,r0,\[Phi]0,\[Theta]s,\[Phi]s,\[Theta]k,\[Phi]k,d}];
...
If[fsampling < fmax, Print["Samplign freq smaller than fmax"];Exit[]];
If[1/2/\[Delta]t < fsampling, Print["Nymquist criterion failed"];Exit[]]; (*fNy = fs/2 > fmax *)
...
(* Build the signal *)
{hplus[t_],hcross[t_]} = emritemplate[m1,m2,spin,\[Alpha],massphi,r0,\[Phi]0,\[Theta]s,\[Phi]s,\[Theta]k,\[Phi]k,observingtime,t]; 
...
\[Rho] = snrrwave[m1,m2,spin,\[Alpha],massphi,r0,\[Phi]0,\[Theta]s,\[Phi]s,\[Theta]k,\[Phi]k,dL,observingtime,12];

Print["SNR = ",\[Rho]];

dhI = dhII = ConstantArray[0,npar]; (* all paramse + dL *)

order = {11,11,11,11,11,11,9,9,9,9};
pos = {1,2,3,4,5,6,7,8,10,11};

If[Length[order]!=Length[pos],Print["Derivatives don't match parameters"];Exit[],Continue];

vecder=Table[finitederiv[d,pos[[i]],
{m1,m2,spin,\[Alpha],massphi,r0,\[Theta]s,\[Phi]s,\[Phi]0,\[Theta]k,\[Phi]k},\[Epsilon],observingtime,order[[i]]],{i,1,Length[pos]}];(* all derivatives but distance and \[Phi]0 *)
...
dhI[[9]]=Thread[{hI[[All,1]],1/d hI[[All,2]]}];
dhII[[9]]=Thread[{hI[[All,1]],1/d hII[[All,2]]}]; 
...
If[Length[wave1]!=Length[wave2],Print["Different # of samples"];Exit[]];
...
Kval = If[thresh<1,Count[Diagonal[S],u_/;u>thresh],Length[Diagonal[S]]-thresh];
...
If[nsample1!=nsample2,Print["Different # of samples"];Exit[]];
