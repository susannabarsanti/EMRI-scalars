...
dEGRdr = Import["mathematica/dEdrGR09HPI.mx"];
dEddr = Import["mathematica/dEdrd09HPI.mx"];
...
findr0[m1_,m2_,spin_,\[Alpha]_,rend_,T_]:=Block[{q=m2/m1,factsec,factday,fact,tend,rini,r,\[ScriptCapitalE],eqns,t},

\[ScriptCapitalE][r_] = dEGRdr[r,spin]+\[Alpha]^2 dEddr[r,spin];
...
OrbitalEvoT[m1_,m2_,spin_,\[Alpha]_,r0_,\[Phi]0_,T_]:=Block[{solr,t,sol,r,\[Psi],factday,factsec,tend,q=m2/m1,fact,eqns,IC,\[ScriptCapitalE]},
...
dopplerf[m1_,m2_,spin_,\[Alpha]_,r0_,\[Phi]0_,\[Theta]s_,\[Phi]s_,T_,t_]:=Block[{M,\[Mu],fdopp,\[Omega],x,evolution,\[Omega]D,fD},

M = m1 msun; (* total mass *)
\[Mu] = m2 msun; (* reduced mass *)

evolution = OrbitalEvoT[m1,m2,spin,\[Alpha],r0,\[Phi]0,T];
...
emritemplate[m1_,m2_,spin_,\[Alpha]_,r0_,\[Phi]0_,\[Theta]s_,\[Phi]s_,\[Theta]k_,\[Phi]k_,T_,t_]:=Block[{evolution,\[Phi],\[Mu],M,\[Phi]D,r,\[Omega],LN},
...
htime[m1_,m2_,spin_,\[Alpha]_,r0_,\[Theta]s_,\[Phi]s_,\[Phi]0_,\[Theta]k_,\[Phi]k_,observingtime_]:=Block[{win,nn,timevec,T,\[Delta]t,t,
...
rmax = OrbitalEvoT[m1,m2,spin,\[Alpha],r0,\[Phi]0,observingtime][[2]][T]; (* final radius after T *)	
risco = KerrIsco[spin]; (* isco radius/M *)
fdopp = dopplerf[m1,m2,spin,\[Alpha],r0,\[Phi]0,\[Theta]s,\[Phi]s,observingtime,T]; (* max fdop @ rmax *)
...
{hplus[t_],hcross[t_]} = emritemplate[m1,m2,spin,\[Alpha],r0,\[Phi]0,\[Theta]s,\[Phi]s,\[Theta]k,\[Phi]k,observingtime,t]; (* analytical waveform *)
...
snrrwave[m1_,m2_,spin_,\[Alpha]_,r0_,\[Phi]0_,\[Theta]s_,\[Phi]s_,\[Theta]k_,\[Phi]k_,dL_,T_,WDT_]:=Block[{fmin=10^-4,rmax,fdopp,fmax,d,\[Rho],hI,hII,timevec,fact,hp,hx,FpI,FxI,FpII,FxII,hIt,hIIt},

d=10^6 dL pc; (* Luminosity distance *)

{timevec,hIt,hIIt}=htime[m1,m2,spin,\[Alpha],r0,\[Theta]s,\[Phi]s,\[Phi]0,\[Theta]k,\[Phi]k,T];

{hI,hII} = fftwave[hIt,hIIt,timevec];

rmax = OrbitalEvoT[m1,m2,spin,\[Alpha],r0,\[Phi]0,T][[2]][T day]; (* final radius after T *)	
fdopp = dopplerf[m1,m2,spin,\[Alpha],r0,\[Phi]0,\[Theta]s,\[Phi]s,T,T day]; (* max fdop @ rmax *)
fmax = cc KerrOrbit[spin,rmax][[1]]/(m1 msun \[Pi])+fdopp; (* f @ rmax + fdop @ rmax *)
...
derwave[\[Epsilon]_,m1_,m2_,spin_,\[Alpha]_,r0_,\[Phi]0_,\[Theta]s_,\[Phi]s_,\[Theta]k_,\[Phi]k_,dL_,observingtime_]:=Block[{timevec,vecder,
...
npar = Length[{m1,m2,spin,\[Alpha],r0,\[Phi]0,\[Theta]s,\[Phi]s,\[Theta]k,\[Phi]k,d}];
...
(* Build the signal *)
{hplus[t_],hcross[t_]} = emritemplate[m1,m2,spin,\[Alpha],r0,\[Phi]0,\[Theta]s,\[Phi]s,\[Theta]k,\[Phi]k,observingtime,t]; 
...
\[Rho] = snrrwave[m1,m2,spin,\[Alpha],r0,\[Phi]0,\[Theta]s,\[Phi]s,\[Theta]k,\[Phi]k,dL,observingtime,12];
...
dhI[[8]]=Thread[{hI[[All,1]],1/d hI[[All,2]]}];
dhII[[8]]=Thread[{hI[[All,1]],1/d hII[[All,2]]}]; 
